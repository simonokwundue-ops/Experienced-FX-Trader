//+------------------------------------------------------------------+
//|                    ForexTrader_v4.0_Enhanced_MultiStrategy.mq5    |
//|          Professional Multi-Strategy EA with Market Regime Logic |
//|       Adaptive Decision Gating & Dynamic ATR-Based Risk Control  |
//+------------------------------------------------------------------+
#property copyright "ForexTrader EA v4.0 Enhanced - Professional Production"
#property link      "https://github.com/simonokwundue-ops/Experienced-FX-Trader"
#property version   "4.00"
#property description "Professional EA with market regime detection & adaptive strategies"
#property description "Hierarchical decision gating: selects best strategy per market condition"
#property description "Dynamic ATR-based lot sizing and volatility-adjusted SL/TP"
#property description "Enhanced signal quality with confluence scoring"
#property description "Multi-symbol portfolio with intelligent risk allocation"
#property strict
...
//+------------------------------------------------------------------+
//| Market Regime Enumeration (NEW in v4.0)                          |
//+------------------------------------------------------------------+
enum ENUM_MARKET_REGIME
{
   REGIME_STRONG_UPTREND = 0,      // Strong Uptrend (Momentum strategies)
   REGIME_STRONG_DOWNTREND = 1,    // Strong Downtrend (Momentum strategies)
   REGIME_RANGING_LOW_VOL = 2,     // Range-bound Low Volatility (Mean reversion)
   REGIME_RANGING_HIGH_VOL = 3,    // Range-bound High Volatility (Cautious)
   REGIME_NEUTRAL = 4              // Neutral/Transitioning (Reduce activity)
};

//+------------------------------------------------------------------+
//| Input Parameters - Multi-Symbol Trading                          |
//+------------------------------------------------------------------+
input group "=== Multi-Symbol Trading (v4.0) ==="
...
//+------------------------------------------------------------------+
//| Input Parameters - Market Regime Detection (NEW in v4.0)         |
//+------------------------------------------------------------------+
input group "=== Market Regime Detection (v4.0) ==="
input bool     UseMarketRegimeFilter = true;   // Enable Market Regime Detection
input ENUM_TIMEFRAMES RegimeTimeframe = PERIOD_H1; // Regime Analysis Timeframe (H1/H4)
input int      Regime_FastSMA = 50;            // Fast SMA for Trend Detection
input int      Regime_SlowSMA = 200;           // Slow SMA for Trend Detection  
input double   Regime_ADX_Trending = 25.0;     // ADX Threshold for Trending Market
input double   Regime_ADX_Ranging = 20.0;      // ADX Threshold for Ranging Market
input double   Regime_ATR_HighVol_Multiplier = 1.5; // ATR multiplier for high volatility
input int      Regime_ATR_Period = 100;        // ATR average period for volatility
...
input int      MinSignalScore = 25;           // Minimum Signal Score (0-100) - REDUCED from 35
input bool     UseAdaptiveScoring = true;     // Use Regime-Adaptive Signal Scoring
input double   SignalConfluenceBonus = 15.0;  // Bonus for multi-strategy confluence
...
input double   MA_SlopeMinimum = 3.0;         // Min MA Slope (Pips) - REDUCED from 5.0
input double   MA_DistanceMinimum = 2.0;      // Min Distance Between MAs (Pips) - NEW
input bool     UseMomentumFilter = true;      // Use 3-Bar Momentum Filter - NEW
input int      MomentumBars = 3;              // Bars for Momentum Calculation - NEW

//+------------------------------------------------------------------+
//| Input Parameters - Breakout Detection (NEW in v4.0)              |
//+------------------------------------------------------------------+
input group "=== Breakout Detection (v4.0) ==="
input bool     UseBreakoutDetection = true;   // Enable Breakout Detection
input int      BreakoutBars = 5;              // Number of Bars for Range
input double   BreakoutThresholdPips = 5.0;   // Min Breakout Size (Pips)
...
input bool     UseVolatilityAdjustedRisk = true; // Adjust risk by volatility (NEW v4.0)

//+------------------------------------------------------------------+
//| Input Parameters - Position Sizing                               |
//+------------------------------------------------------------------+
input group "=== Position Sizing ==="
input double   StopLossPips = 40.0;           // Default Stop Loss (Pips)
input double   TakeProfitPips = 80.0;         // Default Take Profit (Pips)
input bool     UseATRSizing = true;           // Use ATR for Dynamic SL/TP (RECOMMENDED)
input double   ATR_SL_Multiplier = 2.0;       // ATR Multiplier for SL (2.0 = 2x ATR)
input double   ATR_TP_Multiplier = 4.0;       // ATR Multiplier for TP (4.0 = 4x ATR)
input double   MinSL_Pips = 15.0;             // Minimum SL (Pips) - Protection
input double   MaxSL_Pips = 100.0;            // Maximum SL (Pips) - Risk Control
...
input int      MaxDailyTrades = 100;          // Max Trades Per Day (INCREASED for v4.0)
input int      MaxConcurrentPositions = 5;    // Max Concurrent Positions
input int      CooldownMinutes = 1;           // Cooldown Between Trades (Min) - REDUCED to 1
input bool     SeparateCooldownByDirection = true; // Separate Buy/Sell Cooldown
input int      PostCloseCooldownMinutes = 1;  // Cooldown After Position Close (Min) - REDUCED
...
//+------------------------------------------------------------------+
//| Market Regime Detection Variables (NEW in v4.0)                  |
//+------------------------------------------------------------------+
ENUM_MARKET_REGIME currentMarketRegime = REGIME_NEUTRAL;
int handleRegimeFastSMA, handleRegimeSlowSMA, handleRegimeADX, handleRegimeATR;
...
int OnInit()
{
   ...
   //--- Create market regime detection indicators (NEW in v4.0)
   if(UseMarketRegimeFilter)
   {
      handleRegimeFastSMA = iMA(_Symbol, RegimeTimeframe, Regime_FastSMA, 0, MODE_SMA, PRICE_CLOSE);
      handleRegimeSlowSMA = iMA(_Symbol, RegimeTimeframe, Regime_SlowSMA, 0, MODE_SMA, PRICE_CLOSE);
      handleRegimeADX = iADX(_Symbol, RegimeTimeframe, ADX_Period);
      handleRegimeATR = iATR(_Symbol, RegimeTimeframe, Regime_ATR_Period);
      
      if(handleRegimeFastSMA == INVALID_HANDLE || handleRegimeSlowSMA == INVALID_HANDLE ||
         handleRegimeADX == INVALID_HANDLE || handleRegimeATR == INVALID_HANDLE)
      {
         Print("Error: Failed to create market regime indicators");
         return INIT_FAILED;
      }
      
      ArraySetAsSeries(regimeFastSMA, true);
      ArraySetAsSeries(regimeSlowSMA, true);
      ArraySetAsSeries(regimeADX, true);
      ArraySetAsSeries(regimeATR, true);
      
      Print("Market Regime Detection: ENABLED (", EnumToString(RegimeTimeframe), ")");
   }
   ...
   Print("========================================");
   Print("ForexTrader v4.0 Enhanced - Initialized");
   ...
   Print("Market Regime Filter: ", UseMarketRegimeFilter ? "ENABLED" : "Disabled");
   Print("Min Signal Score: ", MinSignalScore);
   Print("Dynamic Lot Sizing: ", UseATRSizing ? "ENABLED (ATR-based)" : "Fixed");
...
void OnDeinit(const int reason)
{
   ...
   //--- Release market regime handles (NEW in v4.0)
   if(handleRegimeFastSMA != INVALID_HANDLE) IndicatorRelease(handleRegimeFastSMA);
   if(handleRegimeSlowSMA != INVALID_HANDLE) IndicatorRelease(handleRegimeSlowSMA);
   if(handleRegimeADX != INVALID_HANDLE) IndicatorRelease(handleRegimeADX);
   if(handleRegimeATR != INVALID_HANDLE) IndicatorRelease(handleRegimeATR);
...
void OnTick()
{
   ...
   if(UseMarketRegimeFilter)
   {
      if(TimeCurrent() - lastRegimeUpdate > regimeUpdateIntervalSeconds)
      {
         UpdateMarketRegime();
...
//+------------------------------------------------------------------+
//| Check Entry Signals (ENHANCED in v4.0)                           |
//+------------------------------------------------------------------+
void CheckEntrySignals()
{
   ...
   //--- Generate signals from each strategy (with regime filtering)
   ...
   if(UseMAStrategy && IsStrategyEnabledForRegime(STRATEGY_MA))
   {
      ...
      if(maScore != 0) strategiesUsed++;
   }
   
   //--- RSI Strategy (check regime)
   if(UseRSIStrategy && IsStrategyEnabledForRegime(STRATEGY_RSI))
   {
      ...
      if(rsiScore != 0) strategiesUsed++;
   }
   
   //--- BB Strategy (check regime)
   if(UseBBStrategy && IsStrategyEnabledForRegime(STRATEGY_BB))
   {
      ...
      if(bbScore != 0) strategiesUsed++;
   }
   
   //--- MACD Strategy (check regime)
   if(UseMACDStrategy && IsStrategyEnabledForRegime(STRATEGY_MACD))
   {
   ...
   //--- Process buy signal
   if(buySignalScore >= threshold && IsCooldownElapsed(true))
   {
      string regimeNames[] = {"STRONG_UPTREND", "STRONG_DOWNTREND", "RANGING_LOW_VOL", "RANGING_HIGH_VOL", "NEUTRAL"};
      Print("=== BUY SIGNAL | Score: ", buySignalScore, " | Threshold: ", threshold, " | Regime: ", regimeNames[currentMarketRegime], " ===");
   ...
   //--- Process sell signal
   if(sellSignalScore >= threshold && IsCooldownElapsed(false))
   {
      string regimeNames[] = {"STRONG_UPTREND", "STRONG_DOWNTREND", "RANGING_LOW_VOL", "RANGING_HIGH_VOL", "NEUTRAL"};
      Print("=== SELL SIGNAL | Score: ", sellSignalScore, " | Threshold: ", threshold, " | Regime: ", regimeNames[currentMarketRegime], " ===");
...
//+------------------------------------------------------------------+
//| Analyze MA Strategy (ENHANCED in v4.0)                           |
//+------------------------------------------------------------------+
int AnalyzeMAStrategy(string &buyReasons, string &sellReasons)
{
   ...
   //--- Check MA distance (NEW in v4.0)
   double maDistance = MathAbs(fastMA[1] - slowMA[1]) / pipSize;
   if(maDistance < MA_DistanceMinimum)
      return 0; // MAs too close together
   
   //--- Check for crossover OR strong trend continuation
   int score = 0;
   bool isBullish = fastMA[1] > slowMA[1];
   bool isBearish = fastMA[1] < slowMA[1];
   ...
   if(bullishCross)
   {
      ...
      //--- Check MA slope for quality
      double maSlope = (fastMA[1] - fastMA[2]) / pipSize;
      ...
      //--- Check momentum filter if enabled (NEW in v4.0)
      if(UseMomentumFilter && CheckMomentum(_Symbol, pipSize) > 0)
      {
         score += 10;
         buyReasons += "Bullish Momentum | ";
      }
      
      //--- Check breakout if enabled (NEW in v4.0)
      if(UseBreakoutDetection && CheckBreakout(_Symbol, pipSize) > 0)
      {
         score += 10;
   ...
   else if(bearishCross)
   {
      ...
      //--- Check MA slope for quality
      double maSlope = (fastMA[2] - fastMA[1]) / pipSize;
      if(maSlope >= MA_SlopeMinimum)
      {
         score -= 10;
   ...
   //--- Trend continuation signals (weaker, only if regime allows)
   if(UseMarketRegimeFilter && currentMarketRegime != REGIME_NEUTRAL)
   {
      if(isBullish && fastMA[0] > fastMA[1] && fastMA[1] > fastMA[2])
      {
         // Strong uptrend continuation
         score = 20;
         buyReasons += "MA Uptrend Continuation | ";
         return score;
      }
      else if(isBearish && fastMA[0] < fastMA[1] && fastMA[1] < fastMA[2])
      {
         // Strong downtrend continuation
         score = -20;
         sellReasons += "MA Downtrend Continuation | ";
         return score;
      }
   }
...
void UpdateMarketRegime()
{
   ...
   if(CopyBuffer(handleRegimeFastSMA, 0, 0, 3, regimeFastSMA) < 3) return;
   if(CopyBuffer(handleRegimeSlowSMA, 0, 0, 3, regimeSlowSMA) < 3) return;
   if(CopyBuffer(handleRegimeADX, 0, 0, 3, regimeADX) < 3) return;
   if(CopyBuffer(handleRegimeATR, 0, 0, Regime_ATR_Period, regimeATR) < Regime_ATR_Period) return;
   
   double currentPrice = iClose(_Symbol, RegimeTimeframe, 0);
   
   //--- Calculate average ATR for volatility classification
   double avgATR = 0;
   for(int i = 0; i < Regime_ATR_Period; i++)
      avgATR += regimeATR[i];
   avgATR /= Regime_ATR_Period;
   
   double currentATR = regimeATR[0];
   bool isHighVolatility = (currentATR > avgATR * Regime_ATR_HighVol_Multiplier);
   
   //--- Determine trend direction
   bool isUptrend = (regimeFastSMA[0] > regimeSlowSMA[0] && currentPrice > regimeFastSMA[0]);
   bool isDowntrend = (regimeFastSMA[0] < regimeSlowSMA[0] && currentPrice < regimeFastSMA[0]);
   bool isTrending = (regimeADX[0] > Regime_ADX_Trending);
   bool isRanging = (regimeADX[0] < Regime_ADX_Ranging);
   
   //--- Classify market regime
   ENUM_MARKET_REGIME previousRegime = currentMarketRegime;
   
   if(isUptrend && isTrending)
      currentMarketRegime = REGIME_STRONG_UPTREND;
   else if(isDowntrend && isTrending)
      currentMarketRegime = REGIME_STRONG_DOWNTREND;
   else if(isRanging && !isHighVolatility)
      currentMarketRegime = REGIME_RANGING_LOW_VOL;
   else if(isRanging && isHighVolatility)
      currentMarketRegime = REGIME_RANGING_HIGH_VOL;
   else
      currentMarketRegime = REGIME_NEUTRAL;
   
   //--- Log regime changes
   if(currentMarketRegime != previousRegime)
   {
      string regimeNames[] = {"STRONG_UPTREND", "STRONG_DOWNTREND", "RANGING_LOW_VOL", "RANGING_HIGH_VOL", "NEUTRAL"};
      Print("Market Regime Changed: ", regimeNames[previousRegime], " -> ", regimeNames[currentMarketRegime]);
      Print("ADX: ", DoubleToString(regimeADX[0], 1), " | ATR: ", DoubleToString(currentATR/pipSize, 1), " pips");
   }
}
...
int CheckMomentum(string symbol, double pipSz)
{
   ...
   if(CopyClose(symbol, PERIOD_CURRENT, 0, MomentumBars + 1, closes) < MomentumBars + 1)
      return 0;
   
   //--- Calculate momentum: current price vs N bars ago
   double momentum = (closes[0] - closes[MomentumBars]) / pipSz;
   
   if(momentum > 3.0) // Bullish momentum (>3 pips in MomentumBars)
      return 1;
   else if(momentum < -3.0) // Bearish momentum
      return -1;
...
int CheckBreakout(string symbol, double pipSz)
{
   ...
   if(CopyHigh(symbol, PERIOD_CURRENT, 1, BreakoutBars, highs) < BreakoutBars) return 0;
   ...
   double rangeSize = (rangeHigh - rangeLow) / pipSz;
   
   //--- Check if range is significant enough
   if(rangeSize < BreakoutThresholdPips)
      return 0;
   
   //--- Check current price vs range
   double currentPrice = iClose(symbol, PERIOD_CURRENT, 0);
   
   if(currentPrice > rangeHigh) // Bullish breakout
      return 1;
   else if(currentPrice < rangeLow) // Bearish breakout
      return -1;
...
//+------------------------------------------------------------------+
//| Calculate Stop Loss (ENHANCED in v4.0)                           |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Calculate Take Profit (ENHANCED in v4.0)                         |
//+------------------------------------------------------------------+
double CalculateTakeProfit(ENUM_ORDER_TYPE orderType, double price)
{
   ...
   if(UseATRSizing && ArraySize(atrBuffer) > 0)
   {
      ...
      if(tpPips < (MinSL_Pips * 1.5)) // TP should be at least 1.5x SL minimum
         tpDistance = (MinSL_Pips * 1.5) * pipSize;
   }
   else
   {
